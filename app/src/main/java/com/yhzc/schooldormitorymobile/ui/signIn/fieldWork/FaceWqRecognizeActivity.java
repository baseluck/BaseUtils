 ...
 * @描述:
 */
public class FaceWqRecognizeActivity extends BaseActivity<FaceViewModel, ActivityFaceRecognizeBinding> {
    ...
    protected void afterCreate(Bundle savedInstanceState) {
        wqly = getIntent().getStringExtra("wqly");
    ...
    private void signIn(String picUrl) {
        ...
        if (StringUtils.isEmpty(wqly)) {
            ToastUtils.showShort("打卡理由不能为空");
            return;
        }
        Map<String, String> map = new HashMap<>();
        map.put("vcAddress", address);
        map.put("vcGoOutReason", wqly);
        map.put("vcHeadIcon", picUrl);
        OkGoUtils.post(ApiUrl.WQDK, map, new BaseCallback() {
            @Override
            protected void onSuccess(String callback) {
