 ...
 * @描述:
 */
public class MeetingSignInNoUserFaceActivity extends BaseActivity<FaceViewModel, ActivityFaceRecognizeBinding> {
                            ...
                            if (baiduFaceModel.getResult().getUser_list().size() != 0) {
                                for (BaiduFaceModel.ResultBean.UserListBean userListBean : baiduFaceModel.getResult().getUser_list()) {
                                    if (userListBean.getScore() > 80) {
                                        updateFace(nv21, width, height, userListBean.getUser_id());
    ...
    private void updateFace(byte[] nv21, int width, int height, String userId) {
                ...
                protected void onSuccess(String callback) {
                    ...
                    if (upLoadBean.getCode() == ApiUrl.SUCCESS) {
                        if (upLoadBean.getData() != null && upLoadBean.getData().size() != 0) {
                            signIn(upLoadBean.getData().get(0).getUrl(), userId);
                        }
                    }
                }
    ...
    private void signIn(String picUrl, String vcJoinUserId) {
        Map<String, String> map = new HashMap<>();
